...
export default function PokerGame() {
  ...
  useEffect(() => {
    ...
    connectSocket(gameId, playerId,
      (data) => {
        ...
        if (data.inGame && !handFetchedRef.current) {
          handFetchedRef.current = true
          setTimeout(() => fetchMyHand(), 1000)
        }
        setGame(data)
  ...
  // Lógica de botones simple — sin all-in
  const canBet   = isMyTurn && !isFolded && !hasActiveBet && myCredit > 0
  const canCall  = isMyTurn && !isFolded && hasActiveBet && myCredit > 0
  const canCheck = isMyTurn && !isFolded && !hasActiveBet
  const canFold  = isMyTurn && !isFolded

  useEffect(() => {
    if (timerRef.current) clearInterval(timerRef.current)
    if (!isMyTurn || !isRunning || isFolded) {
  ...
  if (showBet) {
    ...
    const pcts = [
      ...
      { label: '25%',  val: Math.floor(myCredit * 0.25) },
      { label: '50%',  val: Math.floor(myCredit * 0.5)  },
      { label: 'TODO', val: myCredit },
    ]
    return (
      <div style={s.betOverlay}>
        <div style={s.betModal}>
          ...
          <div style={s.betInfoGrid}>
            ...
            <div style={s.betInfoBox}>
              <span style={s.betInfoLabel}>Pozo</span>
              <span style={{ ...s.betInfoValue, color: '#27ae60' }}>{pot.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div style={s.betSliderWrap}>
            <input
              ...
              max={myCredit}
              step={500}
              value={Math.min(raiseAmt, myCredit)}
              onChange={(e) => {
            ...
            <div style={s.betAmountDisplay}>
              ...
              <input
                ...
                value={raiseAmt}
                onChange={(e) => {
                  const v = Math.min(Number(e.target.value), myCredit)
                  setRaiseAmt(v); raiseAmtRef.current = v; setBetWarning('')
                }}
                min={minBet}
                max={myCredit}
...
const s: Record<string, React.CSSProperties> = {
  ...
  betActions:        { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 },
